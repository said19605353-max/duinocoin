...
def detect_arduino_ports():
    """Return a list of serial port devices that are likely Arduino boards."""
    arduino_vids_pids = [
        ...
        (0x0403, 0x6001),  # FTDI (common)
    ]
    ports = []
    for port in serial.tools.list_ports.comports():
        # Check by hardware VID/PID
        if port.vid is not None and port.pid is not None:
            if (port.vid, port.pid) in arduino_vids_pids:
                ports.append(port.device)
                continue
        # Check description / hwid for "arduino"
...
def check_port_changes():
    """
    Mevcut seri portları kontrol eder, kayıtlı portlarla karşılaştırır.
    Değişiklik varsa kullanıcıya bilgi verir, güncelleme seçeneği sunar.
    Güncellenmiş port ve rig_identifier listelerini döndürür.
    """
    global avrport, rig_identifier, hashrate_list

    current_all_ports = [p.device for p in serial.tools.list_ports.comports()]
    saved_ports = avrport[:]  # mevcut kayıtlı liste (kopya)

    # Hangi kayıtlı portlar hâlâ bağlı?
    present_ports = [p for p in saved_ports if p in current_all_ports]
    # Hangi kayıtlı portlar eksik?
    missing_ports = [p for p in saved_ports if p not in current_all_ports]

    # Yeni Arduino portlarını tespit et (sadece Arduino olduğu bilinenler)
    arduino_ports = detect_arduino_ports()
    new_ports = [p for p in arduino_ports if p not in saved_ports]

    if missing_ports or new_ports:
        print(Fore.YELLOW + Style.BRIGHT + "\n⚠ Port değişikliği tespit edildi!" + Style.RESET_ALL)
        if missing_ports:
            print(Fore.RED + f"Eksik port(lar): {', '.join(missing_ports)}")
        if new_ports:
            print(Fore.GREEN + f"Yeni Arduino port(ları): {', '.join(new_ports)}")

        choice = input(Fore.CYAN + "Yapılandırmayı güncellemek ister misiniz? (e/h): " + Style.RESET_ALL).strip().lower()

        if choice in ('e', 'evet', 'y', 'yes'):
            # Yeni port listesi: hâlâ bağlı olan eski portlar + yeni Arduino portları
            new_port_list = present_ports + new_ports
            new_rig_list = []

            # rig_identifier eşleştirmesi: eski portlar için mevcut isimleri koru
            for port in new_port_list:
                if port in saved_ports:
                    idx = saved_ports.index(port)  # ilk bulduğunu al
                    new_rig_list.append(rig_identifier[idx])
                else:
                    # Yeni port için isim sor
                    name = input(f"  {port} için isim (boş bırakılırsa 'None'): ").strip()
                    if not name:
                        name = "None"
                    new_rig_list.append(name)

            # Küresel değişkenleri güncelle
            avrport = new_port_list
            rig_identifier = new_rig_list
            hashrate_list = [0] * len(avrport)

            # Yapılandırmayı güncelle ve kaydet
            config["AVR Miner"]["avrport"] = ','.join(avrport)
            config["AVR Miner"]["identifier"] = ','.join(rig_identifier)
            with open(str(Settings.DATA_DIR) + '/Settings.cfg', 'w') as configfile:
                config.write(configfile)
            print(Fore.GREEN + "✔ Yapılandırma güncellendi." + Style.RESET_ALL)

        else:
            # Güncelleme istenmezse sadece bağlı olan portlarla devam et
            print(Fore.YELLOW + "Yapılandırma değiştirilmedi. Sadece bağlı portlarla devam ediliyor." + Style.RESET_ALL)
            if missing_ports:
                # rig_identifier listesini de senkronize et (sadece present_ports için)
                present_rig = []
                for port in present_ports:
                    idx = saved_ports.index(port)
                    present_rig.append(rig_identifier[idx])
                rig_identifier = present_rig
                avrport = present_ports
                hashrate_list = [0] * len(avrport)
    else:
        print(Fore.GREEN + "✔ Tüm kayıtlı portlar bağlı, yeni port yok." + Style.RESET_ALL)
...
def load_config():
    ...
    if not Path(str(Settings.DATA_DIR) + '/Settings.cfg').is_file():
        ...
        detected_ports = detect_arduino_ports()
        ...
        if detected_ports:
            ...
            use_auto = input(
                Style.RESET_ALL + Fore.YELLOW
                + f"Found {len(detected_ports)} Arduino port(s). Use them automatically? (Y/n): "
                + Fore.RESET + Style.BRIGHT
            ).strip().lower()

            if use_auto in ('', 'y', 'yes'):
                avrport_list = detected_ports
                rig_identifier = ''
                name_them = input(
                    Style.RESET_ALL + Fore.YELLOW
                    + "Do you want to assign a custom name to each board? (y/N): "
                    + Fore.RESET + Style.BRIGHT
                ).strip().lower()
                if name_them in ('y', 'yes'):
                    for port in avrport_list:
                        name = input(f"  Name for {port}: ")
                        if not name:
                            name = "None"
                        rig_identifier += name + ','
                    rig_identifier = rig_identifier.rstrip(',')
                else:
                    rig_identifier = ','.join(["None"] * len(avrport_list))
            else:
                # User declined auto-detect – fall back to manual selection
                ...
                port_names = [p.device for p in portlist]
                avrport_list = []
                rig_identifier = ''
                while True:
                    current_port = input(
                        Style.RESET_ALL + Fore.YELLOW + get_string('ask_avrport')
                        + Fore.RESET + Style.BRIGHT)
                    if current_port in port_names:
                        confirm_identifier = input(
                            Style.RESET_ALL + Fore.YELLOW + get_string('ask_rig_identifier')
                            + Fore.RESET + Style.BRIGHT)
                        if confirm_identifier.lower() in ('y', 'yes'):
                            current_identifier = input(
                                Style.RESET_ALL + Fore.YELLOW + get_string('ask_rig_name')
                                + Fore.RESET + Style.BRIGHT)
                            rig_identifier += current_identifier
                        else:
                            rig_identifier += "None"

                        avrport_list.append(current_port)
                        confirmation = input(
                            Style.RESET_ALL + Fore.YELLOW + get_string('ask_anotherport')
                            + Fore.RESET + Style.BRIGHT)
                        if confirmation.lower() not in ('y', 'yes'):
                            break
                    else:
                        print(Style.RESET_ALL + Fore.RED + 'Please enter a valid COM port from the list above')
        else:
            # No Arduino ports found → go manual
            print(Style.RESET_ALL + Fore.YELLOW + "No Arduino ports automatically detected.")
            portlist = serial.tools.list_ports.comports(include_links=True)
            for port in portlist:
                print(Style.RESET_ALL + Style.BRIGHT + Fore.RESET + '  ' + str(port))
            print(Style.RESET_ALL + Fore.YELLOW + get_string('ports_notice'))
        ...
        avrport = ','.join(avrport_list)
        # rig_identifier is already a comma-separated string
